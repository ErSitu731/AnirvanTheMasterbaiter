...
local function OnRead(inst, reader)
    if reader and reader.components and reader.components.ragebait_controller then
        reader.components.ragebait_controller:CraftManual("ragebait_manual_tallbird")
        if reader.components.talker then
            reader.components.talker:Say("I have learned to rouse the tallbird's anger!")
        end
...
return Prefab("ragebait_manual_tallbird", fn, assets)